    ...
    providers: [
        CredentialsProvider({
          ...
          async authorize(credentials) {
              ...
              const accessToken = await excuteQuery(`SELECT api_token from tokens WHERE user_id = '${id[0].id}'`)
              return { userName: user[0].username, userID: id[0].id, token: accessToken[0].api_token };
          },
      }),
  ],
  callbacks: {
